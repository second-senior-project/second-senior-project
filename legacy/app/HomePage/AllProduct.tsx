    ...
    <div className="relative flex w-full max-w-[26rem] flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg">
      <div className="relative mx-4 mt-4 overflow-hidden text-white shadow-lg rounded-xl bg-blue-gray-500 bg-clip-border shadow-blue-gray-500/40">
        <img
          ...
          onClick={() => router.push('/HomePage/id', el)}
        />
        ...
        <button className="absolute top-4 right-12 h-8 w-8 bg-black rounded-full text-white flex items-center justify-center"
          onClick={toggleMenu}>
          <IoMdMore className="icon" />
        </button>
        {menuView && (
          <div className="absolute top-12 right-12 bg-white shadow-md rounded-md py-2 w-48">
            <span className="block px-4 py-2 text-gray-700 cursor-pointer hover:bg-gray-100" onClick={() => router.push("/editProduct",el={el})}>Edit Product</span>
            <span className="block px-4 py-2 text-gray-700 cursor-pointer hover:bg-gray-100" onClick={() => deleteProd(el.id)}>Delete</span>
          </div>
        )}
      </div>
      <div className="p-6">
