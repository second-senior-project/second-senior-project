...
export default function Home() {
  ...
  const renderNavBar = () => {
    ...
    if (seller) {
      return <SellerNavBar  toggleMenu={toggleMenu} menuView={menuView} signOut={signOut} router={router} key={key} setKey={setKey} filter={filter}/>;
    } else if (user) {
      return <UserNavBar  toggleMenu={toggleMenu}menuView={menuView} signOut={signOut} router={router} key={key} setKey={setKey} filter={filter}/>;
    } else {
...
const SellerNavBar = ({toggleMenu,menuView,signOut,router,key,setKey}) => (
          ...
          <div className="hidden mr-4 lg:block">
            <ul className="flex flex-col gap-2 mt-2 mb-4 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
              ...
              <li className="block p-1 font-sans text-sm antialiased font-normal leading-normal text-black">
                ...
                <li className="block p-1 font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                  <a href="/Panier" className="flex items-center text-blue-gray-900 text-lg">
                    Panier
                  </a>
                </li>
                <li className="block p-1 font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                  <a href="/Wishlist" className="flex items-center text-blue-gray-900 text-lg ">
                   wishlist
                  </a>
                </li>
              </ul>
...
const UserNavBar = ({ toggleMenu, menuView, signOut, router,key,setKey,filter}) => (
  <div className="grid min-h-[90px] w-full place-items-center overflow-x-scroll rounded-lg p-6 lg:overflow-visible">
    <div className="-m-6 max-h-[768px] w-[calc(100%+48px)] ">
      <nav className="sticky top-0 z-10 block w-full max-w-full px-4 py-2 text-black bg-white border rounded-none shadow-md h-max border-white/80 bg-opacity-80 backdrop-blur-2xl backdrop-saturate-200 lg:px-8 lg:py-4 h-1">
        <div className="flex items-center justify-between text-blue-gray-900 -mt-5">
          <a href="/HomePage" onClick={() => router.push('/HomePage')} className="mr-4 block cursor-pointer py-2 font-sans text-base font-medium leading-relaxed text-inherit antialiased text-xl">
            exclusive
          </a>
